...
export default function AdminDashboard() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      const pOrang = axios.get(`${API_BASE}/api/orang_tua`, { headers: authHeaders }).catch(() => ({ error: true })); // baru

      const [
        rNews, rGallery, rVisi, rSejarah, rPengumuman,
        rTeacher, rHeadmaster, rProgram, rSiswa, rPrestasi, rOrang
      ] = await Promise.all([
        pNews, pGallery, pVisi, pSejarah, pPengumuman,
        pTeacher, pHeadmaster, pProgram, pSiswa, pPrestasi, pOrang
      ]);
      ...
      const orangData = rOrang && !rOrang.error ? (Array.isArray(rOrang.data) ? rOrang.data : []) : []; // baru
      ...
      setCounts({
        ...
        orangTua: orangData.length || 0, // set baru
      });
    ...
    <div className="dashboard">
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-content">
            <h3>Orang Tua</h3>
            <p className="stat-number">{counts.orangTua}</p>
          </div>
        </div>
      </div>
      ...
      <div className="quick-actions">
        ...
        <div className="actions-grid">
          ...
          <button className="action-btn brown" onClick={() => navigate("/admin/siswa")}>
            <span className="action-icon">🎒</span>
            <span>Data Siswa</span>
          </button>

          <button className="action-btn teal" onClick={() => navigate("/admin/orangtua")}>
            <span className="action-icon">👪</span>
            <span>Data Orang Tua</span>
          </button>

          <button className="action-btn red" onClick={() => navigate("/admin/prestasi")}>
            <span className="action-icon">🏆</span>
            <span>Prestasi</span>
          </button>
        </div>
